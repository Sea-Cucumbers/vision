...
public:
    ...
    void upload0(cv::Mat image)
    {
        ...
        /*
        case RS2_FORMAT_Y16:
            glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, width, height, 0, GL_LUMINANCE, GL_UNSIGNED_SHORT, (void*)image.ptr(0,0));
            break;
        */
        case RS2_FORMAT_RGB8:
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, (void*)(image.ptr(0,0)));
            break;
        default:
            throw std::runtime_error("The requested format is not supported by this demo!");
        }
    ...
    void upload1(unsigned char *data, int width, int height, int nrChannels){
        if (!_gl_handle)
            glGenTextures(1, &_gl_handle);
        GLenum err = glGetError();
...
void draw_pointcloud(float width, float height, glfw_state& app_state, std::vector<cv::Point3d>& points, std::vector<cv::Point2d>& pixels, rs2::points& points_i)
{
    //if (!points)
        //return;
    ...
    auto vertices = points;              // get vertices
    auto vertices_i = points_i.get_vertices(); 
    auto tex_coords = pixels; // and texture coordinates
    double w = 640;
    double h = 512;
    double u, v;
    uint64_t num = 0;
    //int flag=0;
    size_t size = points_i.size();
    ...
    for (size_t i = 0; i < size; i++)
    {
        if (vertices_i[i].z  != 0)
        {
            ...
            glTexCoord2d(u, v);
            //glTexCoord2fv((GLfloat *)&(tex_coords[i]));
        }
    }
...
void draw_pointcloud_original(float width, float height, glfw_state& app_state, rs2::points& points)
{
    if (!points)
        return;
    // OpenGL commands that prep screen for the pointcloud
